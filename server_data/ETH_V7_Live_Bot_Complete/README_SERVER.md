...
- 只在这些小时允许出信号；其他小时信号不发出。空数组 `[]` = 不过滤（当前默认，总盈利最优：21,111 笔 / 60.0% / +42,375 / 日均 22.3）。
- 与回测语义一致：被过滤的信号不占冷却位、不计数、不影响每日熔断。
- 2026-08-04 回测扫描曾推荐 {0,1,2,4,5,8,10,17,20}（8,305 笔 / 61.1% / 日均 8.8），但总盈利减半，用户决策不采纳，机制保留便于日后调整。
